<template>
  <div class="home">
    <HomeHeader :gender="gender" />

    <van-swipe
      class="my-swipe"
      :autoplay="3000"
      :show-indicators="false"
      :loop="true"
    >
      <van-swipe-item
        v-for="(item, index) in gender === 'male' ? bannerMale : bannerFemale"
        :key="index"
      >
        <van-image width="100%" :src="item.img"  @click='$router.push("/book/" + item.id)' />
      </van-swipe-item>
    </van-swipe>

    <van-grid>
      <van-grid-item
        icon="data:image/png;base64,"
        text="分类"
        to="/category"
      />
      <van-grid-item
        icon="data:image/png;base64,"
        text="排行"
        to="/ranking"
      />
      <van-grid-item
        icon="data:image/png;base64,"
        text="书架"
         to="/bookrack"
      />
      <van-grid-item
        icon="data:image/png;base64,"
        text="充值"
      />
    </van-grid>

    <HomeList
      v-if="maleHot"
      :booklists="gender === 'male' ? maleHot : femaleHot"
      >{{ gender === "male" ? "男生热门" : "女生热门" }}</HomeList
    >
    <HomeList
      v-if="maleFinish"
      :booklists="gender === 'male' ? maleFinish : femaleFinish"
      >{{ gender === "male" ? "男生完本" : "女生完本" }}</HomeList
    >

    <Bottom></Bottom>

    <!-- <router-link to="/category">分类</router-link> -->
    <!-- <router-link to="/ranking">排行</router-link> -->
  </div>
</template>

<script>
import { mapState } from "vuex";
// @ is an alias to /src
import Bottom from "@/components/Bottom.vue";

import HomeHeader from "@/components/HomeHeader.vue";
import HomeList from "@/components/HomeList.vue";

export default {
  name: "Home",
  components: {
    HomeHeader,
    HomeList,
    Bottom
  },
  data() {
    return {
      gender: "male",
      bannerMale: [],
      bannerFemale: [],
      maleHot: null,
      maleFinish: null,
      femaleHot: null,
      femaleFinish: null
    };
  },
  computed: {
    ...mapState(["rankCategory"])
  },

  created() {
    this.axios.get("/banner.json").then(response => {
      this.bannerMale = response.data.male;
      this.bannerFemale = response.data.female;
    });

    //首页的男女热搜/完本 数据请求
    this.axios
      .get("http://novel.kele8.cn/rank/54d42d92321052167dfb75e3")
      .then(response => {
        // console.log(response);
        this.maleHot = response.data.ranking;
      });

    this.axios
      .get("http://novel.kele8.cn/rank/564eb878efe5b8e745508fde")
      .then(response => {
        // console.log(response);

        this.maleFinish = response.data.ranking;
      });
    this.axios
      .get("http://novel.kele8.cn/rank/54d43437d47d13ff21cad58b")
      .then(response => {
        // console.log(response);
        this.femaleHot = response.data.ranking;
      });

    this.axios
      .get("http://novel.kele8.cn/rank/564eb8a9cf77e9b25056162d")
      .then(response => {
        // console.log(response);
        this.femaleFinish = response.data.ranking;
      });
  },
  beforeRouteEnter(to, from, next) {
    // 在渲染该组件的对应路由被 confirm 前调用
    // 不！能！获取组件实例 `this`
    // 因为当守卫执行前，组件实例还没被创建
    next(vm => {
      vm.gender = to.query.gender || "male";
    });
  },
  beforeRouteUpdate(to, from, next) {
  
    this.gender = to.query.gender || "male";
    next();
  }
};
</script>
